function aur-install
    ...
    function aur-get-pkgbuild
        ...
        if test (curl --progress-bar -sL "https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=$argv" | head -n1) != "<!DOCTYPE html>"
            ...
            logger 4 "! Unable to get the PKGBUILD file of "$argv", please check your package name and network connection, abort"
            exit
        end
        if git clone "https://aur.archlinux.org/$argv"
            ...
            logger 4 "! Unable to get build files of "$argv", please check your package name and network connection, abort"
            exit
        end
    end
    for target in $argv
        if test -e ~/.ctpm/aur/$target/PKGBUILD
            logger 0 '? Detected an existed BUILDPKG file, would you want to remove it and build it from aur?[y/N]'
        ...
        if test "$aur_build_only" = true
            if makepkg -s
                logger 1 "Store at ~/.ctpm/aur/$target/"
            else
                logger 4 "Package $target failed to build"
            end
        else
            makepkg -si
        end
